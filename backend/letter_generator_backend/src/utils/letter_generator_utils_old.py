    ...
    try:
        ...
        for index, row in reporting_df.iterrows():
            try:
                ...
                position_count = row.iloc[7] if len(row) > 7 else 1  # Колонка H (индекс 7)
...
def generate_letter_document(letter_data, output_path):
    ...
    try:
        ...
        main_text = f"""Настоящим сообщаем, что между «{letter_data["be_name"]}» и {letter_data["contractor_full_form"]} «{letter_data["contractor_name"]}» (далее – «{letter_data["contractor_short_name"]}») заключен договор поставки № {letter_data["reg_number"]} от {letter_data["reg_date"]} (далее – Договор поставки). В соответствии с Договором поставки сторонами подписана Спецификация № **{letter_data["order_number"]}** от 03.03.2025 (далее – спецификация), согласно которой «{letter_data["contractor_short_name"]}» обязуется в срок до {letter_data["planned_date"]} поставить товары на сумму {format_amount_in_words(letter_data["total_amount"])}, а «{letter_data["be_name"]}» - оплатить указанные товары в течение 30 (тридцати) календарных дней с момента их передачи (Приложение № 1 к настоящему письму).

По состоянию на {datetime.now().strftime("%d.%m.%Y")} товары в количестве **{letter_data["total_positions"]}** позиций на {format_amount_in_words(letter_data["total_amount"])} в месте поставки {"отсутствуют" if letter_data["category"] == "просрочено не поставлено" else f"поступили с просрочкой"}, что является нарушением п. 4.1 Договора поставки. На основании п. 8.3. Договора поставки сумма пени на текущий момент по просроченным позициям составляет **{format_amount_in_words(letter_data["total_penalty"])}** и рассчитывается следующим образом:
...
Обращаем Ваше внимание на то, что в настоящее время имеется перечень критичных для «{letter_data["be_name"]}» позиций товара (Приложение № 2 к настоящему письму), поставка которых должна быть осуществлена до {letter_data["planned_date"]}, при этом, риски срыва сроков поставки являются недопустимыми.
...
Учитывая изложенное, убедительно просим Вас ускорить исполнение обязательств, принятых по Договору поставки, в части своевременной отгрузки товаров и поставки товара в целях недопущения увеличения суммы пени по позициям товара согласно Приложению № 1 к настоящему письму и минимизации рисков образования пени по позициям товаров согласно приложению № 2 к письму."""
    ...
    try:
        ...
        header_info.add_run(f"Номер заказа: {letter_data['order_number']}\n")
        header_info.add_run(f"Количество просроченных позиций: {letter_data['total_positions']}\n")
        header_info.add_run(f"На сумму: {format_amount_in_words(letter_data['total_amount'])}")
